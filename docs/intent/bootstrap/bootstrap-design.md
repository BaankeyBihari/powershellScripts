...
- `winget`/`msstore` groups carry `items` (always installed) and `optional` (user is prompted once, y/N, for the whole optional batch) as arrays of `{id, name}` objects — `id` is the exact package identifier passed to `winget install --id`/`winget install --source msstore --id`, `name` is a human-readable display name, included because raw identifiers (`marha.VcXsrv`, `9PFHDD62MXS1`) aren't recognizable at a glance. For `winget` entries, `name` is the catalog's `Name` field (e.g. `{"id": "ALCPU.CoreTemp", "name": "Core Temp"}`); for `msstore` entries, `name` is the Store listing's app name (e.g. `{"id": "9PFHDD62MXS1", "name": "Apple Music"}`). `scoop` groups keep `items`/`optional` as flat ID strings — scoped out of this convention (see Decisions & Alternatives). `scoop` groups may also list `buckets` to add before installing.
...
`Test-PackageInstalled` checks whether a given source/item pair is already installed before attempting install, building a `$plan` of `{Source, Item, Name, Required, AlreadyInstalled}` records — `Item` holds the installable identifier (a `winget`/`msstore` entry's `id`, or the flat string for `scoop`); `Name` holds the display name where one exists (`winget`/`msstore` entries) and falls back to `Item` otherwise, so every plan record has something readable to print regardless of source. The pre-install plan listing and per-item install-progress line address the package by `Name` alone (`BOOT-016`) — these are routine, non-error paths where readability wins. The install loop wraps each entry's install command in a `try`/`catch` and, on success, checks `$LASTEXITCODE` via `Test-InstallExitCode`; a non-zero exit code or a thrown exception both add the entry to a `$failed` list, and a per-item warning plus a final summary line are printed once the loop completes, addressing the package by `Name (Item)` — the friendly name followed by the exact identifier passed to the package manager, in parentheses — so a failure is traceable straight back to the `default.json` entry without a separate lookup (`BOOT-017`). `Item` (the real `id`) is always what's passed to `winget`/`scoop` install and lookup commands, regardless of what's shown to the user. Installs are not halted on a single failure — the rest of the plan still runs — but every failure is surfaced (`BOOT-013`).
...
| Item display name | `{id, name}` object per entry, `name` sourced from the winget catalog's `Name` field for `winget` entries and the Store listing's app name for `msstore` entries | Winget's own `Moniker` field; a separate sibling lookup dictionary keyed by id; a generated markdown table outside `default.json` | Raw identifiers (`marha.VcXsrv`, `9PFHDD62MXS1`) aren't recognizable without a lookup. Winget's catalog `Moniker` was rejected because it's absent for many packages (`astral-sh.uv`, `ScooterSoftware.BeyondCompare.4` have none). A sibling dictionary or external doc was rejected because it decouples the name from its id with nothing enforcing the pair stays in sync as entries are added or removed — an inline object keeps them atomic. |
| `name`'s scope: `winget` and `msstore`, not `scoop` | `items`/`optional` adopt `{id, name}` for both `winget` and `msstore` groups; `scoop` stays flat ID strings | `winget` only; uniform `{id, name}` across all three item-bearing sources including `scoop` | Both `winget` package IDs (`marha.VcXsrv`) and `msstore` Store product IDs (`9PFHDD62MXS1`) are equally opaque without a lookup, so both get the name field. `scoop`'s only current item (`sudo`) is already self-explanatory, so extending the convention there wasn't warranted. |
...
| Display format for `Name` vs `Item` | Plan listing and progress lines show `Name` alone; failure warnings and the final summary show `Name (Item)` | One format everywhere (either name-only or name+id) | Plan/progress output is a routine, non-error path where the friendly name is all that's needed; failure output is where a reader needs to trace straight back to the exact `default.json` `id` without a separate lookup, so the id stays visible there. |
...
3. ✅ `msstore` items adopt the same `{id, name}` shape as `winget` — see the Decisions table.
4. ✅ `WhatsApp.WhatsApp`, a dead winget ID (no longer present in the catalog), is removed from `winget.optional` rather than repaired — no live winget listing for WhatsApp was found; msstore's `9NKSQGP7F2NH` remains a separate, unregistered candidate if WhatsApp is wanted back via that source.
