...
- [x] **TEE-005**: If `Invoke-TeeCommand` is invoked with no command to run — either zero arguments total, or only `-LogPath <path>` with nothing after it — then the system shall report a clear error naming what's missing, before any log-path setup happens, rather than failing on an unguarded null index.
